  ...
  module Adapter
    def self.valid_credentials?(login, password_plaintext)
      options = {:login => login,
                 ...
                 :ldap_auth_username_builder => ::LDAP.ldap_auth_username_builder,
                 :admin => ::LDAP.ldap_use_admin_to_bind}

      resource = LDAP::Connection.new(options)
      resource.authorized?
    end
    ...
    def self.update_own_password(login, new_password, current_password)
      set_ldap_param(login, :userPassword, ::LDAP.ldap_auth_password_builder.call(new_password), current_password)
    end
